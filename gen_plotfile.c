...
int main(int argc, char *argv[]) {
...

  {
    ...
    for (dayno = 0; dayno < num_days; dayno++) {
      ...
      level_costs = 0.01 * (100 * costs_dollars[dayno] + costs_cents[dayno]);
      level_margin = 0.01 * (100 * margin_dollars[dayno] + margin_cents[dayno] + 100 * costs_dollars[dayno] + costs_cents[dayno]);

      level_revremain = 0.01 * (100 * revenues_dollars[dayno] + revenues_cents[dayno]);

      printf("%ld %g %g %g\n", dayno, level_costs, level_margin, level_revremain);
